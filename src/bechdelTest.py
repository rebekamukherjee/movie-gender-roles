...
female_names = "../data/bechdel/female.txt"
male_names = "../data/bechdel/male.txt"
male_names = read_name_file(male_names)
female_names = read_name_file(female_names)
parser = IMSDBParser(male_names+female_names, n_scripts=20)
...
print (int(np.average([len(set([m[0] for m in movie])) for movie in complete_successful_movies])), int(np.average([len(set([m[0] for m in movie])) for movie in unsuccessful_movies])))
print ("Average number of male characters for each script (successful/unsuccessful):")
print (int(np.average([len(set([m[0] for m in movie if m[0] in male_names])) for movie in complete_successful_movies])),int(np.average([len(set([m[0] for m in movie if m[0] in male_names])) for movie in unsuccessful_movies])))
print ("Average number of female characters for each script (successful/unsuccessful):")
print (int(np.average([len(set([m[0] for m in movie if m[0] in female_names])) for movie in complete_successful_movies])),int(np.average([len(set([m[0] for m in movie if m[0] in female_names])) for movie in unsuccessful_movies])))
